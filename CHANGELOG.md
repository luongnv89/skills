...
### Added
- **`opencode-sandbox` 1.4.0** (was `opencode-docker-dev` 1.3.0, unreleased): run OpenCode inside a [luongnv89/docker-dev](https://github.com/luongnv89/docker-dev) container without mounting `~/.ssh` or injecting a GitHub token. The container is **kept by default** (`sleep infinity` as PID 1). Agents use **`--start-only` then `--exec-in`** so a copy-paste `docker exec -it <name> zsh` attach command reaches the user before OpenCode blocks; after reviewing the host diff, the agent **asks before `docker rm`**. `--rm` on the exec invocation restores auto-remove for one run. Do not use for opencode.ai cloud routing (`opencode-runner`), Herdr pane management, or running the project's own app.
