...
### Added
- **`plan-to-issues` 1.5.0:** carries a finished plan into the tracker. Reads `MODERNIZATION_PLAN.md` (`/codebase-modernizer`) or a `/tasks-generator` `tasks.md`, files **one issue per plan task** through `/issue-creator` batch mode bound with `--parent <epic>`, and makes the **epic body the dashboard** — every child grouped by phase, per-phase progress bars, milestone status, critical path, and a *Next actionable* list. Labels are derived by rule, not judgement: `phase:`, type, `dim:` per closed finding dimension, `priority:` from the report's severity (phase default when no report). It is **plan-faithful** — it never opens source files, so nothing it writes violates `/issue-creator`'s Output Contract — and **idempotent**: a task already filed under the epic is skipped, so a rate-limited run resumes by re-running. An 11-check **Phase 0 preflight** gates the run *before the first mutation*, reporting every failure in one pass with its fix. Six of the checks are `gh` **readiness**, not presence: authenticated as the intended account (it asks when two are logged in), `repo` token scope, a repo with issues enabled and not archived, write permission (`TRIAGE` degrades to "apply existing labels only", `READ` stops), an unambiguous target when several remotes exist, and API budget for `4 × tasks + 20` requests. The rest cover git, `python3`, the `issue-creator` skill, this skill's own bundled files, and a resolvable plan. Missing-skill blocks carry the `asm install` command plus `npm install -g agent-skill-manager` for `asm` itself. `/plan-to-issues sync <epic#>` re-renders the dashboard from live issue states via `scripts/render_dashboard.py`, rewriting only the region between the `plan-dashboard` sentinels and refusing to touch an epic that has none. Plan text is treated as untrusted throughout: the renderer collapses newlines in **every** plan-derived string (headings, goals, task lines — not only table cells, where it also escapes `|`), and plan-derived titles are *sourced* from the parsed worklist via `jq` rather than typed into any shell literal — `gh` has no `--title-file`, so the bound-variable form is the only safe one. The epic is bound by a **plan-binding marker** (`<!-- plan-to-issues:plan=<path> -->`) written immediately after creation and before the first child is filed. It cannot ride in on the create call — `/issue-creator` reproduces intent text verbatim *in a blockquote*, so a marker sent that way lands `> `-prefixed and mid-body — and there is no atomic create-with-marker, so the short create/bind window is handled by **recovery** rather than eliminated: an unmarked epic is offered for adoption on the next run after an explicit confirm. Render-input validation now covers every consumed field — container *and* element types, plus scalar guards on the fields that reach dict lookups — so a malformed input exits 2 with a hint rather than raising (96 malformed shapes verified). `epic` and `issue` must be positive integers and `synced` an ISO date: as free strings they let a caller forge a premature `plan-dashboard:end`, fake dashboard checklist rows that sync reads back as real children, and a second `**Progress:**` line.
