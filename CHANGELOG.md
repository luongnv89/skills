...
### Features
- **subagent-creator**: New skill at **1.0.1** — create, evaluate, or improve Claude Code subagent definition files (`.claude/agents/*.md` or `~/.claude/agents/*.md`), based on the `luongnv89/claude-howto/04-subagents` guideline. A branch-selector skill: **Create** scaffolds a single-responsibility agent with least-privilege tools and the proven role → when-invoked → process → output-format → checklist body; **Evaluate** runs a six-category design-audit rubric (frontmatter validity, single responsibility, least-privilege tools, description/trigger quality, body structure, anti-patterns) and reports before/after fixes without editing; **Improve** fixes an existing file in impact order (description → over-reach → tools → body) and re-checks. Ships `references/frontmatter-schema.md` (every field, common-vs-advanced, tool-restriction syntax, file precedence — advanced/version-dependent fields flagged verify-before-use, no hardcoded version numbers), `references/system-prompt-guide.md` (single-responsibility test, annotated ground-truth example, description-writing, Do/Don't), `references/evaluation-rubric.md`, and two ready-to-copy templates (`reviewer-agent.md` read-only, `worker-agent.md` editing). Includes a Repo Sync Before Edits guardrail for the project-scope (`.claude/agents/`) path. Description trimmed to 241 chars so the negative-trigger clause survives `/skills`-listing truncation; doesn't trigger for skills (`skill-creator`), `CLAUDE.md`/`AGENTS.md` (`agent-config`), or running an agent. Cleared the skill-creator standard (`quick_validate` clean, frontmatter audit, SKILL.md 159 lines). **1.0.1** corrects the frontmatter-schema against the official subagents doc: removes the non-existent `context: fork` field (forking is a `/fork`/`CLAUDE_CODE_FORK_SUBAGENT` session feature, not frontmatter) and adds the real `color` field; fixes the `model` default to `inherit` and adds the `fable` alias; relaxes the `name`-must-match-filename claim to a recommended convention (identity comes from `name`, not the filename) so the rubric no longer false-fails a valid agent; completes the `permissionMode` (`auto`) and `effort` (`xhigh`) enums; and adds managed/enterprise settings as the top precedence tier.
