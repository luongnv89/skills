...
### Added
- **`opencode-handoff` 1.0.0**: hand a limit-blocked OpenCode session to a fresh sandbox. Builds an `opencode-sandbox` container for the same project with the global (`~/.agents`) and project-local (`<repo>/.agents`) agent setup mounted, deliberately **excluding** the host's OpenCode config, token, and key (`~/.config/opencode/service.json`, `~/.local/share/opencode/auth.json`), then opens a **detached** tmux session on it and surfaces the `attach-session` line plus the `opencode auth login` step. OpenCode inside starts unauthenticated on purpose — that is what supplies the fresh usage allowance. The skill asserts the credential boundary against the running container and **refuses to hand over** on a leak. Don't use for one-shot sandbox tasks (`opencode-sandbox`), opencode.ai routing (`opencode-runner`), or Herdr panes.
...
### Changed
- **`herdr-agent-comms` 1.23.0 → 2.0.0:** rebuilt on the Herdr 0.9 agent surface, and fixed a **command that does not exist**. Seven call sites across `references/delivery-and-waiting.md` used `herdr wait agent-status <pane> --status <state>`; the real CLI is `herdr agent wait <target> [--until STATUS]`, so Phase 4's delivery check and Phase 5's helper-free waiter could not run as written. The test suite passed because `tests/fake_herdr.py` implemented the invented command.

  The rewrite deletes machinery rather than adding it. `herdr agent prompt <target> "<task>" --wait --timeout MS` submits and waits in **one** server-side request: it refuses a `blocked` target with `agent_blocked` before writing any input, sends text plus encoded Enter as one ordered write honoring bracketed paste, gates on observed activity (`agent_prompt_stalled` if none), then waits for the first settled state. The pre-send transcript baseline and the split `HERDR_DONE_` completion marker existed only to approximate that from outside across two calls, and are gone. `herdr agent start <name> --kind KIND --pane ID` returns only when Herdr has detected the agent and considers it interactive-ready, replacing `pane run` plus `agent rename` plus a concurrent `wait_for_idle.py --ready` pass. Verified live against herdr 0.9.0: `agent start` returned ready in 3.6s, a prompt round trip in 2.5s.

  What survives is the check the server does **not** make. `agent prompt` refuses `blocked` but accepts `working`, and its wait tracks lifecycle state rather than one turn, so a prompt into a working agent can be satisfied by the turn already in flight and you read back the wrong reply. `preflight_send.py` keeps that fail-closed refusal, now on the agent surface, and gained exit 5 for `agent_not_found` — verified live, a pane whose process Herdr does not recognize reports `agent_status: unknown` on the *pane* surface but is not addressable on the *agent* surface at all, so `wait_for_idle.py` stays as the documented fallback for exactly that case.

  **New: monitoring and reporting**, which the skill had none of. `scripts/fleet_status.py` renders the whole fleet from one `herdr api snapshot` call — flat cost for any fleet size, and a consistent view where N per-agent `agent get` calls can report a state that never existed at one instant — sorted so blocked agents come first, with `--fail-on-blocked` exiting 3. `scripts/badge.py` wraps `herdr pane report-metadata` so each worker's sidebar row carries its assigned job, role and phase; it is display-only and never perturbs waits or rollups, and it exists because that call takes a pane id while fleet bookkeeping runs on agent names. `herdr notification show` reaches the human when an agent blocks or a run completes. `references/fleet-monitoring.md` is new; `broadcast.sh` was rebuilt on concurrent `agent prompt --wait` with one-call target resolution and per-code outcome mapping (`BLOCKED`/`STALLED`/`TIMEOUT`/`GONE`), plus opt-in badging via `HAC_BADGE=1`. It also rechecks each target's status immediately before its own dispatch: the roster is read once at T0, resolving and badging N targets takes real time, and Herdr refuses a target that turned `blocked` in that window but not one that turned `working`. Workers are now spawned with `--env HERDR_ROLE=<name>` so an agent can read its own role rather than being told it in every prompt.

  Phase 6 is new (Monitor and Report), pushing broadcast/steer/teardown to 7 and HANDOFF to 8. `compatibility` now names herdr 0.9.0 or later, since the agent surface and `api snapshot` are load-bearing. The fake CLI was rewritten to the real command set — errors as JSON on stderr with exit 1, matching the real thing — and 135 tests pass, including new suites for `fleet_status.py` and `badge.py`.

  The rewrite also reached `wait_for_idle.py`, which had kept its own copy of the invented command in `wait_status()`. Because that call always failed and the two loops around it carried no pause of their own — they were written assuming it blocked — a `working` pane produced a hot spin: 149 `herdr` subprocess spawns at 85% CPU over a 5s wait, extrapolating to roughly 3,500 at the 120s default, on the very path the skill documents as the `agent_not_found` fallback. There is no pane-surface status wait to swap in (`herdr pane wait-output` matches transcript text, not lifecycle state), and the agent surface is by definition unavailable here, so the loop now polls `herdr pane get` on a bounded 0.25s tick. Three regression tests pin it: one bounds the call count on each of the two former call sites, and a third asserts no `herdr wait` subcommand is ever invoked. All three fail if the fix is reverted.

- **`herdr-agent-comms` 2.0.0 → `herdr-agent` 3.0.0 — renamed, plus a `help` option.** The `-comms` suffix described the first version of the skill, which only sent and read messages; it has managed the whole fleet lifecycle since 2.0.0, so the directory, the frontmatter `name`, the H1, the completion-report title, the CI workflow (`.github/workflows/herdr-agent-tests.yml`) and every cross-skill reference are now `herdr-agent`. Four sibling skills name it and were bumped with it: `issue-work-loop` 1.4.0 → 1.4.1 (including the functional `req=` dependency-preflight string, which would otherwise fail the gate on a machine that has the skill installed), `opencode-sandbox` 3.0.0 → 3.0.1, `opencode-handoff` 1.0.4 → 1.0.5, and `tmux-agent-comms` 2.3.0 → 2.3.1. `tmux-agent-comms` keeps its own name. The `HAC_TIMEOUT` / `HAC_LINES` / `HAC_BADGE` environment variables in `broadcast.sh` are **deliberately unchanged** — they derive from the old initials, but renaming them would break anyone's existing environment for no behavioural gain. Installed copies keep the old directory name until users re-run `install.sh`; nothing removes `~/.claude/skills/herdr-agent-comms` automatically, so an unremoved copy will trigger alongside the new one.

  **New: a `help` option.** `/herdr-agent help` — or any plain question about what the skill does — resolves as the first branch in the workflow table, **above** the prerequisite check, and answers with a usage summary covering the nine operations (spawn, message, read, status, broadcast, steer, teardown, handoff, help), the safety behaviours, and the version and environment requirements. It runs **no** `herdr` command, inspects no pane, mutates nothing, and emits no step completion report. Ordering it ahead of Prerequisites is the whole point: that section exits on a failed `herdr status`, and someone asking how the skill works is often someone who has not installed Herdr yet, so a help request that requires a running server answers nobody. A narrower question is answered from its one phase rather than by printing the whole block. A ninth eval case pins the behaviour, and the skill's 135 tests still pass.

- **`herdr-agent` 3.0.0 → 3.1.0: workers mirror the main agent by default.** Before this change, every spawn named its agent kind, and the examples hardcoded `claude` or `pi --thinking low`. A worker therefore ran whatever CLI the orchestrator picked, on that CLI's config default model and effort. Now every sub-agent, HANDOFF successor included, starts on the **inherited launch profile**: the same harness kind, model, thinking level and setup flags as the agent running the skill. The user overrides it per worker by naming a kind, model, thinking level or native flag.

  The profile can't come from one probe, because Herdr only sees part of it. `herdr agent get <root>` reports the kind, and `herdr pane process-info` returns the root pane's argv, which carries launch flags. Neither carries a model or effort chosen mid-session, and config defaults differ from the live values: on the machine this was built on, the main agent ran at effort `max` while `~/.claude/settings.json` said `medium`, and its argv was a bare `claude`. So the main agent reports its own values (`--main-model`, `--main-thinking`), and Claude Code's exported `CLAUDE_EFFORT` is read directly when the script runs inside the root pane. If a value can't be read it is recorded as `UNKNOWN`, not treated as an error: the worker starts on its config default and the summary names the field.

  New `scripts/launch_profile.py` resolves the profile and, with `--start`, runs `herdr agent start` on it. Arguments pass to the subprocess as a list, so nothing is marshalled through a shell array. Four rules keep it safe:
  - **Kind gate.** Model, thinking level and flags are inherited only when the worker's kind matches the main agent's, since a Claude model ID means nothing to `pi`.
  - **Verified flag tables.** Root argv is filtered through per-kind allowlists checked against `--help` for claude 2.1.274, pi and codex 0.153.4. Session, resume, print, prompt, subcommand, credential (`pi --api-key`) and unrecognized flags are dropped and named. Kinds without a table inherit only the kind.
  - **Values stay unprinted.** Setup-flag values never appear in the output, because inline settings or MCP JSON can carry secrets.
  - **Bypass is disclosed.** Permission-bypass flags are inherited, because the orchestrator can already run commands in any worker pane and a bypass-less worker would stall on a dialog Rule 7 forbids it to answer. They are never inherited silently: a `⚠ permission bypass inherited from main` line has to reach the user before the first split, and `--without bypass` (or `--without flags`) opts out.

  Verified live against herdr 0.9.1: `agent start` quotes each native arg (`claude --model 'claude-opus-5[1m]' --effort max` reached the pane intact), and a worker started this way reported `Opus 5 (1M context)·max` within 4s. The fake CLI gained `pane process-info` and now records what `agent start` launched. 19 new tests pin both failure modes that matter (a spawn blocked by an UNKNOWN model, and a Claude model ID handed to a `pi` worker) along with argv filtering, config-key bypass detection and flag-stem-only output, and all 154 tests pass. SKILL.md gains Rule 10, Phase 2 and Phase 8 now start workers on the profile, `references/launch-profile.md` is new, and the CI workflow byte-compiles the new script. Eval 4 now expects the profile, and evals 10 and 11 pin default inheritance and cross-kind non-inheritance.

### Fixed
- **Catalog retrofit sweep — 12 skills brought to the skill-creator standard (#127, #129, #130, #139, #140, #141, #143, #145, #146, #148, #155, #159):** one `skill-auto-improver` Mode 1 run per skill. All twelve **PASS** both hard gates. Three cleared both gates at baseline with a clean predictability audit and are correctly left unbumped — `herdr-agent-comms` (#139), `idea-validator` (#140) and `landing-page-generator` (#143); verifying that is the outcome those issues asked for, and `skill-auto-improver` Phase 5 bumps once per iteration *that produced edits*, so a no-op run bumps nothing. One **Gate 1 failure** was found and fixed. The other eleven changes close advisory Phase 2b findings, and every one of them is a contradiction that would make two agents run the same skill differently, not a style note.

  **`viral-product-evaluator` 1.2.4 → 1.3.0** — the Gate 1 failure. The skill invokes `/browse` on the live-URL path but carried no dependency preflight: Prerequisites named only codebase read access, a landing-page signal, `references/*.md` presence and user confirmation. The new section carries the checklist's four required elements and sits above `## Inputs`, ahead of the first `/browse` mention rather than ahead of the Phase 3 write. It reuses the verified gstack form `dont-make-me-think` already ships in this repo — detection by install path first, because `/browse` ships in gstack and can be present without `asm` knowing about it, so an `asm list` check alone would nag on every live-URL review; `asm install github:garrytan/gstack:browse -p claude -s global --yes` is deliberately not the plain `asm install browse` the template would suggest. It is fail-soft, not fatal: on a miss the run asks for a local file or saved HTML and never scores a URL it could not load. Separately, whether `viral-evaluation.md` must be written was stated three ways, so a URL-only run — explicitly permitted by Inputs — could pass its own Phase 3 report either writing the file or printing inline only; all four sites now say always write, defaulting to the working directory when there is no repo.

  **`opencode-runner` 1.4.2 → 1.5.0** — Phases 5 and 6 re-derived `LOG=/tmp/opencode-$$.log` and referenced `$OPENCODE_PID`, but every Bash call is a new shell: `$$` resolves to a different number each time and `OPENCODE_PID` is unset. Monitoring therefore polled a log that did not exist and cleanup killed nothing and removed nothing, while still reporting "Cleanup complete". A `.pid` file written beside a timestamped run log now carries the process id between calls, and Phase 4 prints a `run=` value the later phases substitute literally.

  **`ollama-optimizer` 1.1.2 → 1.2.0** — six findings. The workflow defined three phases while the report templates enumerated four, promoting Verification (a `####` subsection inside Phase 3) to a peer step, so a run emitted three or four reports depending on which site it read; Verification is now a real Phase 4. `detect_system.py` already computes and emits `hardware_tier`, but SKILL.md omitted that field from the parse list and had the agent re-classify from a table whose bands left 12-16GB and 24-48GB VRAM undefined — the script assigns those prosumer and workstation. Phase 1 now parses `hardware_tier` and Phase 2 overrides it only with a stated reason, and the table's bands match the script. Acceptance criteria demanded env vars written to a shell init file *with a backup* and a one-command rollback, but no step instructed writing or backing up anything; the Execution Checklist now does both. A benchmark was to be compared "against tier baseline from Phase 2", where no tokens/s baselines existed — the dangling comparison is dropped rather than filled with invented numbers. The opt-in fast path sat after the entire workflow it bypasses, so a top-down reader had already run it; it now leads `## Workflow`. And the max model for an 8GB Mac was 8B in two places and 7B in two others.

  **`brand-name-checker` 1.3.2 → 1.4.0** — four findings. The social platform set differed in three places, all claiming "6 platforms"; canonicalized on Twitter/Instagram/GitHub/LinkedIn/TikTok/Discord. The Early-Exit trigger named a "primary platform" that is never defined, against two other sites saying any platform. Step numbering disagreed — six steps in SKILL.md, whose Step 6 (Recommendation) is the early-exit jump target, against templates saying "step N of 5" that folded Recommendation into Risk Assessment. And an early-exit run structurally could not meet its own acceptance criteria, which required the registry, domain and trademark checks "completed" while Steps 2-4 are skipped; those three are now qualified.

  **`tad-generator` 1.4.0 → 1.5.0** — Phase 8 said to write `tad.md`, which Phase 5 had already created and Phase 7 had already committed and pushed, leaving the "working tree clean after push" criterion unsatisfiable if Phase 8 were taken literally. Three subagent files ship and the Subagent Architecture table names them, but SKILL.md never referenced the `agents/` path or said to spawn anything — Phase 4 just said "conduct the 5 research rounds", so one agent spawned three subagent types while another reasoned all five rounds inline in main context. Modification Mode offered a "Scaling" area and said to map it to a matching numbered Phase 5 section, but scaling is subsection 6.2 inside Infrastructure and has no section of its own. Also clarified that `scripts/update_readme_ideas_index.py` belongs to the user's ideas repo, not to this skill.

  **`install-script-generator` 2.2.2 → 2.2.3** — Phase 2's completion bar was unmeetable: `--dry-run` skips every step, so nothing "executes cleanly" and no rollback ever triggers, and the resulting all-`skipped` report is then the default input `doc_generator.py` renders `USAGE_GUIDE.md` from. The documented custom-prefix one-liner set `INSTALL_PREFIX` on `curl` rather than on the piped `bash`, so the installer never saw it despite the "Honours `INSTALL_PREFIX`" requirement. And Phase 2 resolved the dependency graph but invoked `plan_generator.py` without `--deps`, silently dropping it from `installation_plan.yaml`; the flag is `nargs="*"`, verified against the script.

  **`oss-ready` 1.2.2 → 1.3.0** — the Workflow defines eight steps (0-7) but every report and summary shape said "4 of 4", leaving steps 5 (project metadata) and 6 (`.gitignore`) outside the model even though the acceptance criteria require both. An agent tracking "4 of 4" could declare PASS having skipped verifiable work.

  **`aso-marketing` 1.2.2 → 1.3.0** — step-completion reports were mandated after every step of a 7-phase workflow, but only four groupings existed and they stopped at Phase 6, so Phase 7 (Summarize) had no defined checks and the `[step N of M]` denominator was undefined at 7 or 4. A Phase 7 grouping was added and `M` pinned at 5 in both sites.

  **`cli-builder` 1.0.5 → 1.1.0** — two rival exit-code contracts for the same decision: SKILL.md prescribed 0/1/2 with bad input as 1, while `references/testing-patterns.md` added 3 for input errors and 130 for interrupted. The reference is now canonical and SKILL.md points at it instead of restating a different contract.

  Gate 2 is measured over the seven rubric categories `skill-auto-improver` names. `asm` 2.18 adds `license`, `pii` and `script-lint`; `license` scores 5 for **every** skill in this repo, merged retrofits included, because it wants a `LICENSE` file inside each skill directory. No `LICENSE` files were added — fifty new files is scope these issues do not ask for, and no skill in the catalog has one.
- **`issue-work-loop` 1.3.1 → 1.4.0 (#142):** retrofitted to the current skill-creator standard. Both gates failed at baseline; both are green now. **Gate 1** was missing its dependency preflight: the skill hands whole phases to `herdr-agent-comms` (every pane spawn, send and wait) and `issue-pr-review` (the reviewer role) in both modes, plus `issue-resolver` for the ISSUE implementer, and Prerequisites 5-6 named those skills without an install command, an installer bootstrap or a verification step — so a run on a machine missing one failed at the spawn rather than at the gate. The preflight now sits above the repo sync, the first mutating step. **Gate 2** was `asm eval` 84, below the >85 floor, with `testability` 3 and `context-efficiency` 7 against the ≥8 per-category bar. Testability went 3 → 10 by folding the five scattered per-phase `Done when:` blocks into one `## Acceptance Criteria` section — five separate criteria were five chances to disagree — and adding `### Expected output` plus a rename of `Failure Handling` to `## Edge Cases`, which is what that table always was. Context-efficiency went 7 → 8 on two moves and one clarification: the Autonomous Worker Boot Gate inlined the full per-harness matrix that `references/loop-protocol.md` already carries verbatim, and two sources of truth for a safety gate is the worse defect, so the body keeps the invariants and points at the authoritative reference; `Leading Words` and the `work_loop.*` config table are lookup material and moved to a new `references/vocabulary-and-config.md`; and `work_loop.context_threshold` was documented as "FRESHEN at or above this percentage" without ever saying a percentage of what — it is the worker's remaining context window, and a worker that exhausts its token budget mid-review returns a truncated verdict rather than an error. Final: `asm eval` 92, all seven rubric categories ≥ 8 (min 8), `quick_validate.py` exit 0, body 272 lines / 2387 words against the 500 / 3000 caps. Nothing was deleted — every line cut from the body landed in `references/`. Gate 2 is measured over the seven rubric categories `skill-auto-improver` names; `asm` 2.18 adds `license`, `pii` and `script-lint`, and `license` scores 5 for every skill in this repo, merged retrofits included, because it wants a `LICENSE` file inside each skill directory. No `LICENSE` files were added.
- **`website-agent-readiness` 1.1.0 → 1.2.0 (#160):** retrofitted to the current skill-creator standard. **Gate 1 was already green** at baseline — `quick_validate.py` exit 0, clean frontmatter, a negative-trigger clause in the description, the AI-skip notice on `docs/README.md`, and a correct dependency preflight for `/plan-to-issues` — which is what #160 set out to verify. **Gate 2** was not: `asm eval` 86 with `testability` 5 and `context-efficiency` 7 against the ≥8 per-category bar. Testability went 5 → 10 by folding the four per-phase `Completion criteria:` blocks into one `## Acceptance Criteria` section, adding `### Expected output` naming the two artifacts and the tracker state a run actually leaves behind, and consolidating a `## Edge cases` table out of six behaviours that were scattered through the phases or stated only inside a script: an unreachable `localhost` or password-walled staging host, several URLs in one request, a site with zero failing checks (`triage_scan.py` exits 3 and writes nothing, because `/plan-to-issues` rejects a file with no task headings), `isCommerce: false` deferring the P4 commerce tasks, an empty `fixes.md` degrading to the `nextLevel` prompts, and a `neutral` check never becoming a task. Context-efficiency went 7 → 8 by moving `Leading terms` — lookup material — to `references/leading-terms.md`, and by saying *why* the run leaves raw scan data in `.agent-ready/`: the full 22-check response is far larger than the digest the run reasons over, and reading it wholesale burns context window the later phases need. Final: `asm eval` 92, all seven rubric categories ≥ 8 (min 8), `quick_validate.py` exit 0, body 312 lines / 2325 words against the 500 / 3000 caps. Nothing was deleted — every line cut from the body landed in `references/`. Gate 2 is measured over the seven rubric categories `skill-auto-improver` names; `asm` 2.18 adds `license`, `pii` and `script-lint`, and `license` scores 5 for every skill in this repo, merged retrofits included, because it wants a `LICENSE` file inside each skill directory. No `LICENSE` files were added.
- **YAML frontmatter safety — `metadata.author` quoted in 9 skills:** the plain scalar `Luong NGUYEN <luongnv89@gmail.com>` carries `<`, `>` and `@` and violated CLAUDE.md hard rule 2; it is now double-quoted, matching the rest of the catalog. Patch bumps: `auto-push` 1.0.3 → 1.0.4, `appstore-review-checker` 1.2.1 → 1.2.2, `aso-marketing` 1.2.1 → 1.2.2, `fork-upstream-sync` 1.3.2 → 1.3.3, `dont-make-me-think` 1.4.0 → 1.4.1, `install-script-generator` 2.2.1 → 2.2.2, `ollama-optimizer` 1.1.1 → 1.1.2, `oss-ready` 1.2.1 → 1.2.2, `opencode-runner` 1.4.1 → 1.4.2.
- **`prd-generator` 1.3.2 → 1.4.0 (#150):** retrofitted to the current skill-creator standard, then restructured to close the advisory findings the retrofit surfaced. Gate 2 was **already green at baseline** — `asm eval` 97 (A) with every category ≥ 8 (min 9), and the auto-fixer's dry-run reporting "No fixes needed" — but Gate 1 was not: `metadata.author` was an **unquoted** plain scalar carrying `<`, `>` and `@`, three characters on the Frontmatter Audit's must-quote list and two of them named independently by this repo's `CLAUDE.md` hard rule #2. `quick_validate.py` exits 0 on it because the value *parses* — YAML treats those characters as ordinary mid-scalar — which is why check 10 is one of the rows the audit marks as needing an agent read; same defect and same fix as `release-manager` in #151, `seo-ai-optimizer` in #153, `frontend-design` in #138, `test-coverage` in #157 and `doc-manager` in #135. `name: prd-generator` was deliberately left unquoted: the hyphen is mid-token and unambiguous as a plain scalar, and only 2 of ~50 sibling skills quote `name:`. It correctly carries **no** dependency preflight — a deliberate scan of `SKILL.md` and all `references/*.md` for slash-command invocations, `~/.claude/skills/` and `~/.agents/skills/` reads, `asm install`, and phases handed to a named skill returned **zero hits**; the `idea-validator` mention is provenance of the input folder rather than an invocation, and `scripts/update_readme_ideas_index.py` lives in the *user's* ideas repo, so adding a preflight would misstate the runtime dependency surface. The `Bundled scripts` Gate 1 row is likewise n/a: there is no `scripts/` directory.

  The **phase count contradicted itself in four places**, and the disagreement was load-bearing rather than cosmetic. Ground truth is **seven** phases: `### Phase 6` (README maintenance) and `### Phase 7` (commit and push) carry real procedural content, not report boilerplate, so the five templates' `step N of 7` labels were the *correct* sites and the prose was wrong. But both of those phases were nested as `###` under `## Step Completion Reports` rather than under `## Workflow`, while the Mode-selection line and an acceptance criterion both said "Phases 1-5" — so an agent reading top-down finished at Phase 5 and silently skipped updating the ideas-repo README and committing/pushing the PRD it had just generated. Fixing one site alone would have deepened the contradiction, so all four were corrected together: Phases 6 and 7 moved under `## Workflow` immediately after Phase 5, the Mode-selection line now reads "proceed through Phases 1-7 in order" and routes a modify run back into the workflow at Phase 6 (a modify run still has a README to update and a commit to push), and the acceptance criterion now requires reports for phases 1-7. The `step N of 7` labels were left untouched, being already right.

  Separately, `## Step Completion Reports` inlined five near-identical phase templates over ~29% of the body. They were **moved, not deleted**, to a new `references/step-reports.md`, which also gained the two templates Phases 6 and 7 never had — so the seven-phase workflow now has seven report blocks. The generic report format and its `√`/`×`/`—` legend stay in the body above a one-line pointer. Body 254 → 199 lines, 1516 → 1330 words (caps 500 / 3000). `asm eval` holds at **97 (A)** with all seven categories byte-identical to baseline (10/10/9/9/10/10/10, min 9) and `quick_validate.py` exits 0 — the restructure bought context back without costing a single category point. PASS at iteration 2 of 8. The evaluator's one `topSuggestion` (add a labelled `## Example` section) was deliberately not acted on — `context-efficiency` is at 9, and Phase 3 forbids chasing points a passing category does not need. **One advisory finding is left open by design:** the Mode 2 delegability finding — Phase 3 (Clarify Requirements) is not delegable because it needs the user mid-step — because a delegation conversion is opt-in and requires explicit user confirmation.
- **`opencode-handoff` 1.0.0 → 1.0.1 (#175):** credential boundary check now fails closed on a bind-mount of the host OpenCode config/data dirs (realpath-resolved, so a `skills/` symlink to the parent is a leak) and `find`s `auth.json`/`service.json` under every OpenCode config/data tree in the container (`/root`, `/home/*`, `/workspace`), not only three hardcoded `/root` paths. The SKILL.md AC matches the check.
- **`opencode-sandbox` 2.1.0 → 2.1.1 (#175):** Standard mounts table and one-shot Step 1 no longer say `~/.config/opencode` is "always" mounted; it is default-on, skippable with `--no-opencode-config`. Credentials log line documents the two new fields.
- **`devops-pipeline` 2.2.0 → 2.2.1 (#133):** monorepo local Node/Python hooks must `--prefix` / path into the package dir; `files:` does not change pre-commit cwd.
...
### Changed
- **`tmux-agent-comms` 2.2.0 → 2.3.0 (#158):** ran `skill-auto-improver` Mode 1 and closed both advisory Phase 2b findings the first pass had left open. Both hard gates were **already green at baseline** — `quick_validate.py` exit 0, `asm eval` 97 (A) with every category ≥ 8, Frontmatter Audit 11/11, `docs/README.md` AI-skip comment present, all three bundled scripts printing descriptive errors on stderr, and no dependency preflight (correctly, since the skill invokes none — `herdr-agent-comms` appears once as a routing redirect, not a call). The work was the advisory audit. **Duplication:** `wait_for_idle.py`'s exit-code semantics were restated three times in three different orders — Critical Rule 2, Phase 1, Phase 4 — three copies of one code table with nothing keeping them in sync. Consolidated into a single canonical **exit-code table** at the top of `references/delivery-and-waiting.md` covering both `preflight_send.py` and `wait_for_idle.py`, with the shared mental model stated once (0 good, 1 environment error, 2 not-yet, 3 blocked) and the rule that only `preflight_send.py` exit 0 authorizes a send; all three body sites now name *the exit-code table* and point at it instead of restating it. **Progressive disclosure:** `Step Completion Report` and `Edge Cases` were inlined in full, which every run paid for whether or not an edge case fired. Moved to a new `references/reporting-and-edge-cases.md` — report block, `√ × — ⚠` legend, and a new per-operation row table (spawn, message, status, inspect, broadcast, teardown, HANDOFF), plus the eight edge cases restructured from a bullet list into a condition→handling table. **No content was deleted**; both sections landed in `references/` per the retrofit's own rule, and the body keeps a one-line pointer at each heading so the structure signal survives. Body 214 → 192 lines, 1778 → 1694 words (caps 500 / 3000). Both gates held across every iteration: `asm eval` 97 (A) unchanged with all seven categories unchanged (Structure 10, Description 10, Prompt engineering 10, Context efficiency 8, Safety 10, Testability 10, Naming 10), `quick_validate.py` exit 0, and the skill's own 17 tests still pass. `context-efficiency` did not move off 8 — the evaluator's remaining suggestion is generic — but the finding was structural, not score-chasing, and the rule against chasing points a passing category does not need forbids going further. **Still open, deliberately:** the Mode 2 delegability finding — Phases 3 and 4 are heavy but not delegable, since each depends on live pane state and on the previous step's exact baseline file and minted marker suffix. Mode 2 is a restructuring that requires explicit user confirmation and is out of scope here.
- **`tasks-generator` 1.3.1 → 1.3.2 (#156):** `skill-auto-improver` Mode 1 retrofit. Both hard gates were already green at baseline (`quick_validate.py` exit 0; all 11 frontmatter checks; `asm eval` 96 (A) with min category 9; 266 lines / 1486 words; no dependency preflight required, since the four `agents/*.md` are subagents and the skill invokes no other skill), so the run was a verification rather than a repair. One advisory Phase 2b finding was closed: `## Quality Checks` restated four bars that `## Acceptance Criteria` already owned (no circular dependencies, critical path identified, every task linked to the PRD, ambiguous items flagged), so a future edit could update one list and leave the other stale. The bars now live once, in `## Acceptance Criteria`, and `## Quality Checks` points at them plus `references/self-test.md`; the one bar unique to the old list — the explicit MVP vs post-MVP split — was moved into `## Acceptance Criteria` rather than dropped. Body 266 → 260 lines, 1486 → 1480 words, all seven `asm eval` categories unchanged. Two findings stay **open and advisory**: no `allowed-tools` is declared (deliberate — the key *restricts* the tool set, and the name of the tool that spawns the four parallel workers could not be established with confidence from the skill or the repo, so an under-specified list would break the skill at runtime with no eval signal), and the Mode 2 delegability finding (no worker is handed a named `references/` slice as its `Input`), which needs explicit user opt-in and is out of scope here.
- **`plan-to-issues` 1.6.0 → 2.0.4 (#171, #149):** the skill no longer requires a plan file, and no longer treats `MODERNIZATION_PLAN.md` as the expected input. Two input kinds are now first-class and produce identical output — issues plus a tracking epic: a **plan file at any path** (any producer, no filename special-casing), and **conversational intent** taken from the user's own turns after a discussion, with no file written and none created. Resolution is ordered and never guesses: an explicit path wins, then `--from-conversation`, then *both available → ask once*, then plan discovery, then conversational fallback, then a `✗ No input to convert` preflight failure naming all three ways to supply one. An unparseable plan file is a **stop**, never a silent downgrade to the conversation. Because a discussion is not a structured plan, the conversation path cannot be plan-faithful, so it gets a **mandatory draft-and-confirm gate** with no auto-accept in its place: the worklist is drafted from the user's turns under the same no-enrichment contract, printed row by row, and confirmed (`[Y]es / [e]dit / [n]o`) before the first mutation — the file path has a reviewable artifact on disk, this path has only that gate. The confirmed draft is written verbatim into the epic body under a fenced `## Source` heading, which is the conversation path's durable record of what was converted. The design risk this issue actually turns on is **epic identity**, not input parsing: epic binding and idempotent re-run key off a normalized `plan_path`, and a conversation has no stable path. Rather than pretend otherwise, the conversation path binds `<!-- plan-to-issues:conversation=<slug> -->` (slug of the confirmed epic title) and declares re-run idempotency **operator-supplied**: a slug hit is *never* silently reused — the existing epic is printed and reuse is offered — the final report always prints `/plan-to-issues --from-conversation --epic <n>` as the resume handle (which restores the worklist from plan-grammar `## Source`), and `--from-conversation --epic <n>` binds directly and skips discovery, because the issue number is the stable handle and the slug is only a hint that finds it. Two unrelated discussions of one subject slug identically, which is exactly why the prompt exists. The file path keeps `<!-- plan-to-issues:plan=<path> -->` **unchanged**, so epics created by 1.x keep resolving, and a mutual-exclusion probe enforces that an epic carries exactly one input kind. The skill **never materializes a plan file** to obtain an identity — that would breach its own acceptance criterion that `git status --porcelain` match the pre-run snapshot. Filed alongside the **skill-creator standard retrofit (#149)**, which the same edit had to satisfy: the two issues collide on one file and the retrofit's word cap pulls against the new feature's additions, so the input path was added first and the cap enforced over the combined text. Gate 2 **failed at baseline** — `asm eval` 93 (A) but `context-efficiency` **6/10**, below the ≥ 8 floor, root-caused to a body over the 3000-word cap (3969), which is also the Gate 1 breach; one defect, both gates. Fixed by moving prose out rather than deleting it, per the retrofit's own rule: new `references/glossary.md` (full term definitions), `references/security-boundary.md` (the injection and shell-interpolation rules), `references/phase-contracts.md` (unabridged Phases 0–6), `references/acceptance-criteria.md` (the full run-success contract), `references/reporting.md` (report format, per-phase check names, output shapes for both input kinds), and `references/input-resolution.md` (the new input contract). Body 429 → 356 lines, 3969 → ~2960 words. `asm eval` 93 → **94 (A)**, `context-efficiency` 6 → 8, `testability` held at 10 — the interim 7 was the `## Example` rename, since the evaluator scores an `## Expected Output` heading; `quick_validate.py` exits 0. The description was rewritten for the new trigger surface (dropping "Run after `/codebase-modernizer`", which is now one way to get a plan file rather than the expected one) and trimmed under the 250-character runtime budget so the negative-trigger clause survives tail-first truncation. Five evals added for the new paths: conversation-only, arbitrary plan path, both-inputs-present, slug collision, and no-input-at-all. **Advisory Phase 2b findings:** `asm eval`'s remaining suggestions (add a section literally headed `## Example`; favour imperative voice) were not acted on — the first conflicts with the `## Expected Output` heading `testability` scores, and both serve categories already clear of the ≥ 8 floor, which Phase 3 forbids chasing past.
- **`opencode-sandbox` 2.1.0**: two additive mount flags, defaults unchanged. `--no-opencode-config` skips the `~/.config/opencode` mount (which carries `service.json`'s `password`) so a container can run on its own OpenCode account; `--with-agents` mounts `~/.agents` read-only **without** `~/.claude`, and alongside `--no-opencode-config` also re-mounts `~/.config/opencode/skills` read-only so its relative skill symlinks still resolve in the container. Enables `opencode-handoff`.
- **`doc-manager` 2.0.2 → 2.0.3 (#135):** retrofitted to the current skill-creator standard. Gate 2 was **already green at baseline** — `asm eval` 96 (A) with every category ≥ 8, and `asm eval --fix --dry-run` reporting "No fixes needed" — but Gate 1 was not: `metadata.author` was an **unquoted** plain scalar carrying `<`, `>` and `@`, three characters on the Frontmatter Audit's must-quote list and two of them named independently by this repo's `CLAUDE.md` hard rule #2. `quick_validate.py` exits 0 on it because the value *parses* — YAML treats those characters as ordinary mid-scalar — which is why check 10 is one of the rows the audit marks as needing an agent read; same defect and same fix as `release-manager` in #151, `seo-ai-optimizer` in #153, `frontend-design` in #138 and `test-coverage` in #157. It correctly carries **no** dependency preflight: a full-directory scan for slash-command invocations and `~/.claude/skills/` reads returns only prose slashes (`deploy / release / setup`, `library/API/web/CLI/service`), documented-repo paths (`scripts/validate-<name>.sh`, `docs/DECISIONS.md`) and its own fallback branch name `feat/doc-manager` — no skill is invoked, and its only externals are `git` and optional `mmdc`, both already gated by `## Prerequisites`, so adding a preflight would misstate the runtime dependency surface. The `Bundled scripts` Gate 1 row is likewise n/a: there is no `scripts/` directory, because the runbook validator is a **template** the skill emits into the *documented* repo rather than a script it ships. `asm eval`'s two `topSuggestions` (move large sections into `references/`; add a labelled `## Example` section) were deliberately not acted on — both would *add* to a body already inside both caps, and the categories they serve already clear the ≥ 8 floor that Phase 3 forbids chasing past. **No content was cut**: the body is 165 lines / 1733 words against the 500 / 3000 caps, so nothing needed to move to `references/`. `asm eval` holds at 96/A, min category 8; PASS at iteration 1 of 8. **Advisory Phase 2b findings, left open by choice:** the runbook acceptance rule ("not exit 0 at all costs" — agent-satisfiable checks must pass, genuine operator prerequisites may leave `--check` non-zero) is stated at near-full length in four places (SKILL.md:106, :114, :133 and `references/runbook-validation.md`:95) instead of one canonical home with three pointers, which the audit records under both *progressive disclosure* (co-location) and *duplication*. It was left standing because the CHANGELOG records that exact bar as the 2.0.1 correction — the emphasis is load-bearing history, and collapsing it to buy points for a category already at 8 risks re-opening a closed regression. The delegability sub-check **passes** — step 3 already names its `references/` slice and step 2's slice is the citation rule every run reads anyway — so there is no Mode 2 candidate.
