...
- **`herdr-agent-comms` 1.23.0 → 2.0.0:** rebuilt on the Herdr 0.9 agent surface, and fixed a **command that does not exist**. Seven call sites across `references/delivery-and-waiting.md` used `herdr wait agent-status <pane> --status <state>`; the real CLI is `herdr agent wait <target> [--until STATUS]`, so Phase 4's delivery check and Phase 5's helper-free waiter could not run as written. The test suite passed because `tests/fake_herdr.py` implemented the invented command.
  ...
  **New: monitoring and reporting**, which the skill had none of. `scripts/fleet_status.py` renders the whole fleet from one `herdr api snapshot` call — flat cost for any fleet size, and a consistent view where N per-agent `agent get` calls can report a state that never existed at one instant — sorted so blocked agents come first, with `--fail-on-blocked` exiting 3. `scripts/badge.py` wraps `herdr pane report-metadata` so each worker's sidebar row carries its assigned job, role and phase; it is display-only and never perturbs waits or rollups, and it exists because that call takes a pane id while fleet bookkeeping runs on agent names. `herdr notification show` reaches the human when an agent blocks or a run completes. `references/fleet-monitoring.md` is new; `broadcast.sh` was rebuilt on concurrent `agent prompt --wait` with one-call target resolution and per-code outcome mapping (`BLOCKED`/`STALLED`/`TIMEOUT`/`GONE`), plus opt-in badging via `HAC_BADGE=1`. It also rechecks each target's status immediately before its own dispatch: the roster is read once at T0, resolving and badging N targets takes real time, and Herdr refuses a target that turned `blocked` in that window but not one that turned `working`. Workers are now spawned with `--env HERDR_ROLE=<name>` so an agent can read its own role rather than being told it in every prompt.
  ...
  Phase 6 is new (Monitor and Report), pushing broadcast/steer/teardown to 7 and HANDOFF to 8. `compatibility` now names herdr 0.9.0 or later, since the agent surface and `api snapshot` are load-bearing. The fake CLI was rewritten to the real command set — errors as JSON on stderr with exit 1, matching the real thing — and 130 tests pass, including new suites for `fleet_status.py` and `badge.py`.
