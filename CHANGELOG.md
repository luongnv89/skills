...
### Fixed
- **`prd-generator` 1.3.2 → 1.3.3 (#150):** retrofitted to the current skill-creator standard. Gate 2 was **already green at baseline** — `asm eval` 97 (A) with every category ≥ 8 (min 9), and the auto-fixer's dry-run reporting "No fixes needed" — but Gate 1 was not: `metadata.author` was an **unquoted** plain scalar carrying `<`, `>` and `@`, three characters on the Frontmatter Audit's must-quote list and two of them named independently by this repo's `CLAUDE.md` hard rule #2. `quick_validate.py` exits 0 on it because the value *parses* — YAML treats those characters as ordinary mid-scalar — which is why check 10 is one of the rows the audit marks as needing an agent read; same defect and same fix as `release-manager` in #151, `seo-ai-optimizer` in #153, `frontend-design` in #138, `test-coverage` in #157 and `doc-manager` in #135. `name: prd-generator` was deliberately left unquoted: the hyphen is mid-token and unambiguous as a plain scalar, and only 2 of ~50 sibling skills quote `name:`. It correctly carries **no** dependency preflight — a deliberate scan of `SKILL.md` and all four `references/*.md` for slash-command invocations, `~/.claude/skills/` and `~/.agents/skills/` reads, `asm install`, and phases handed to a named skill returned **zero hits**; the `idea-validator` mention is provenance of the input folder rather than an invocation, and `scripts/update_readme_ideas_index.py` lives in the *user's* ideas repo, so adding a preflight would misstate the runtime dependency surface. The `Bundled scripts` Gate 1 row is likewise n/a: there is no `scripts/` directory. **No content was cut**: the body is 254 lines / 1516 words against the 500 / 3000 caps, so nothing needed to move to `references/`, and all four existing reference pointers resolve. `asm eval` holds at 97/A, min category 9; PASS at iteration 1 of 8. The evaluator's one `topSuggestion` (add a labelled `## Example` section) was deliberately not acted on — `context-efficiency` is at 9, Phase 3 forbids chasing points a passing category does not need, and the skill already routes a concrete skeleton to `references/expected-output.md`. **Advisory Phase 2b findings, all three left open by choice:** the phase count contradicts itself in four places — the Mode-selection line and an acceptance criterion both say "Phases 1-5" while the five report templates say "step N **of 7**", and `### Phase 6` (README maintenance) and `### Phase 7` (commit and push) are nested under `## Step Completion Reports` rather than `## Workflow`, so an agent reading top-down can stop at Phase 5 and skip both. It looks like a two-character heading promotion but is not: fixing the heading alone without the other three sites leaves a *worse* contradiction, and fixing all four is a restructure — the wholesale rewrite the audit's no-bloat rule forbids inside a Mode 1 loop, and it would break this PR's atomicity. Recommended as a separate follow-up. Related, `## Step Completion Reports` inlines five near-identical templates over ~29% of the body, a natural `references/step-reports.md`; that is a progressive-disclosure and delegability finding, which routes to **Mode 2** by rule, never a Mode 1 edit. The delegability sub-check otherwise holds — Phase 3 (Clarify Requirements) is legitimately non-delegable because it needs the user mid-step, and Phase 4 already names its slice (`references/prd-template.md`).
