...
It compares `git status --porcelain`, the full `git diff`, and a **SHA-256 manifest of every git-visible path (tracked plus untracked, honouring the target's `.gitignore`)** against the pre-run snapshot, exempting a declared-artifact allowlist (`--allow` adds to it; a trailing slash means a directory **at the repo root** and everything under it, so a per-project `obj/` needs its own `--allow src/App/obj/`, and it exempts **untracked** paths only — rewriting a committed `dist/bundle.js` is still a breach, and so is untracking one, since a path tracked at snapshot time or after the run counts as tracked). The manifest is the load-bearing check: `git diff` cannot see an untracked file, so a newly written `.github/workflows/ci.yml` or test file would otherwise go unnoticed. Assertions that only a transcript can settle are reported as `[MANUAL] ... SKIPPED (run by operator)` rather than silently dropped.
