...
- Script lives in the documented repo's `scripts/`, and the runbook section links to it. On a read-only tree, emit the script inline **and run it once** so you can report its `--check` exit code.
- Run it (`--check`). Each failure you diagnose and fix → append the symptom + cause + fix to `docs/troubleshooting.md`, cited.
...
5. **Runbook**: each runbook section links a `validate-<name>.sh` that exits 0 on `--check` (run it to confirm — on a writable tree it's committed; on a read-only tree it's emitted inline and run once); `docs/troubleshooting.md` reflects any fix applied.
...
- [ ] **Runbook validated.** Each deploy/process/setup section has a check-only `validate-<name>.sh` that exits 0 on `--check` (run to confirm), is linked from the section, and gates every destructive step. On a read-only tree the script is emitted inline and its `--check` exit code reported instead of committed. `docs/troubleshooting.md` records any fix applied.
...
- **Read-only repo**: emit docs and the validation script as a diff/inline summary instead of writing files; still run the script once to report its `--check` exit code.
...
- **Validate the run** — `No unresolved FLAGs`, `Links resolve`, `Inventory closed`, `Runbook exits 0`
